# CANKORA — Project Context for AI Assistants
...
CANKORA (Enterprise Operations Platform by Cankonix) adalah modular monolith untuk operasi PMO. Operational MVP mencakup Project Management Monitoring; target produk berikutnya adalah PMO National Project Control Tower Level 1-3 dengan strategi hybrid native CANKORA + Power BI.
...
Operational rule: P0 is complete (P0-001..P0-014 `Done`). Start P1 per ticket dependency. Do not call CANKORA Control Tower feature-complete until the relevant P1/P2/P3 tickets are `Done`; a mockup or updated requirement document is not implementation evidence.

Current phase: **Phase 4 - Project Intelligence & Command** (P2 complete) + **Phase 7 - Advanced Integrations** (P3 in progress). All P2 tickets (P2-001 through P2-011) are `Done`. `CANKORA-P3-001` is `Done`: BIM/Digital Twin Integration Foundation — migration `000027_bim_models`, backend module `integration/bim` (model/service/handler), 10 routes at `/api/v1/integrations/bim/models`, `ResourceBIMIntegration` seeded, frontend page `/integrations/bim`, smoke test `docs/smoke-test-p3-001-bim.sh` 23/23 PASS (2026-08-28). `CANKORA-P3-002` is `Done` (2026-08-28): Government Entity Resolution — PENDING_MATCH → MATCHED end-to-end (migration `000029`, `resolver.go`, 6 routes, frontend "Resolusi Entitas" tab, smoke 13 checks). `CANKORA-P3-003` is `Done` (2026-08-28): Official Data Validation & Approval Workflow — Data Governance (module `governance/`, migrations `000030`–`000033`, 12 routes, frontend `/governance`, smoke 17/17 PASS). **Do not start IoT next.** `CANKORA-P3-004` (IoT telemetry ingestion) is intentionally moved to the last/future advanced-integration slot. Before IoT, finish governance review blockers, dashboard/GIS regression fixes, read-model/reporting reconciliation, and any remaining non-IoT Phase 3 tickets selected by backlog audit. Do not advance to IoT or a later phase unless dependency and phase gate in `docs/PHASED-IMPLEMENTATION-PLAN.md` are satisfied.

Planning note:
- Kanban task board minimal is part of `CANKORA-P0-010` and should live in the project detail task UI.
- Gantt/timeline is tracked separately as `CANKORA-P1-009`; implement a simple read-only task/milestone timeline first, with drag date editing deferred until the timeline is stable.
...
- **Hybrid BI**: CANKORA is the operational system of record; Power BI reads a governed analytics model and never writes operational tables.
...
```
/Users/harmanto/Documents/Code/DEV/Project Management/CANKORA/
```
...
Immediate order:
1. P0 hardening complete (`CANKORA-P0-014` Done): project-child soft delete cascade, dashboard exclusion, smoke cleanup, orphan soft cleanup — all verified.
2. `CANKORA-P1-001` (Issue management) Done & verified.
3. `CANKORA-P1-016` visual/runtime stabilization Done & verified (2026-08-25).
4. `CANKORA-P1-002` Risk Management Done & verified (2026-08-25): tenant-scoped register, probability/impact, score/severity recompute, mitigation, owner, lifecycle, dashboard/Command Center source mapping.
5. `CANKORA-P1-003` Budget Monitoring Done & verified (2026-08-25): budget line CRUD tenant-safe, variance/usage_pct/status backend-computed, dashboard BUDGET_THRESHOLD warning reads active lines only (`pb.deleted_at IS NULL`), audit `budget.created/updated/deleted`.
6. `CANKORA-P1-004` vendor/contract Done & verified (2026-08-26): vendor/consultant master tenant-scoped, contract CRUD end-to-end di project detail, validation (enum, unique per org, nilai/date), audit, dan smoke API/UI lulus. Contract value belum otomatis mengubah `project.budget_total` (menunggu P1-011 snapshot/validasi).
7. `CANKORA-P1-005` document management Done & verified (2026-08-26): upload/list/get/download/update/delete soft metadata; local disk storage `backend/storage/documents` (RELATIVE key di DB, MinIO/S3 swap-ready); extension allowlist + magic bytes + ukuran; delete menghapus record + file fisik; audit `document.uploaded/updated/downloaded/deleted`; frontend panel "Dokumen" di project detail (summary totals, upload form, list, download/edit/delete, states, footer note menunggu P1-012).
8. Next: `CANKORA-P1-006` corrective action (deviation, root cause, recommendation, PIC, target date, follow-up status, evidence).
...
```bash
# Setup env
cp '/Users/harmanto/Documents/Code/DEV/Project Management/CANKORA/backend/.env.example' \
   '/Users/harmanto/Documents/Code/DEV/Project Management/CANKORA/backend/.env'
# Edit .env: DB_HOST=localhost, DB_PASSWORD sesuai setup lokal

cd '/Users/harmanto/Documents/Code/DEV/Project Management/CANKORA/backend'
